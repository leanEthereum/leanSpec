...
@pytest.mark.parametrize(
    "finalized_slot_val, candidate_slot_val, expected",
    [
        # Perfect squares at/beyond float64 precision boundary (2^53)
        # 2^52 = 4503599627370496 (safe for float)
        pytest.param(
            0,
            4503599627370496,
            True,
            id="delta_2_pow_52_perfect_square",
        ),
        # 2^54 = 18014398509481984 (beyond float precision)
        pytest.param(
            0,
            18014398509481984,
            True,
            id="delta_2_pow_54_perfect_square",
        ),
        # 2^62 = 4611686018427387904 (way beyond float precision)
        pytest.param(
            0,
            4611686018427387904,
            True,
            id="delta_2_pow_62_perfect_square",
        ),
        # Non-justifiable large values
        pytest.param(
            0,
            18014398509481985,
            False,
            id="delta_2_pow_54_plus_1_not_justifiable",
        ),
    ],
)
def test_is_justifiable_after_large_values(
    finalized_slot_val: int, candidate_slot_val: int, expected: bool
) -> None:
    """
    Tests integer arithmetic for large slot values beyond float64 precision.

    These tests verify that the integer-based implementation correctly handles
    large Uint64 values where floating-point arithmetic would lose precision.
    Float64 can only represent integers exactly up to 2^53; beyond that,
    precision is lost and checks would be unreliable.
    """
    finalized_slot = Slot(finalized_slot_val)
    candidate_slot = Slot(candidate_slot_val)
