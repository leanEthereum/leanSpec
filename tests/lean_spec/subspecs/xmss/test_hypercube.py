...
@st.composite
def small_hypercube_with_layer_and_index(draw):
    w = draw(st.integers(min_value=2, max_value=6))
    v = draw(st.integers(min_value=1, max_value=6))
    assume(w ** v <= 6000)
...
def test_property_layer_sizes_conservation_monotonicity(wv):
    ...
    assert all(info.prefix_sums[i] <= info.prefix_sums[i + 1] for i in range(len(info.prefix_sums) - 1))


@given(small_hypercube_with_layer_and_index())
@settings(max_examples=200)
def test_property_map_vertex_roundtrip(params):
    """Tests that map_to_vertex returns valid coords in layer d and map_to_integer inverts it correctly."""
    w, v, d, x = params
    vertex = map_to_vertex(w, v, d, x)
...
def test_property_bijectivity_enum(wv):
    """For tiny hypercubes, map_to_vertex enumerates exactly w**v distinct vertices."""
    w, v = wv
...
def test_property_map_oob(wv):
    """map_to_vertex should raise ValueError when x is equal to the layer size (out-of-range)."""
    w, v = wv
    info = prepare_layer_info(w)[v]
    for d, size in enumerate(info.sizes):
        if size > 0:
...
def test_property_find_layer_prefix(wv):
    """hypercube_find_layer returns (d, r) consistent with prefix sums and global index."""
    w, v = wv
    info = prepare_layer_info(w)[v]
    total = w ** v
...
@settings(max_examples=200)
def test_property_layer_injectivity(params):
    """Ensure injectivity inside a layer for map_to_vertex. This is a randomized test — we pick two distinct indices and assert their mapped vertices differ."""
    w, v, d, x1, x2 = params
